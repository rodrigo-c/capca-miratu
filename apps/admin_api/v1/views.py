...
class PublicQueryAPI(PublicQueryBase):
...
class PublicQueryManager(PublicQueryBase):
    def list(self, request) -> Response:
        data_list = self.filter_by_user(
            public_queries=public_queries_services.get_public_query_list()
        )
    ...
    def api_config(self, request) -> Response:
        token, _ = Token.objects.get_or_create(user=request.user)
        paths = {
            "list": reverse("admin_api:v1:public-query-api-list"),
            "results": reverse(
                "admin_api:v1:public-query-api-detail", kwargs={"pk": "--url_code--"}
            ),
            "geojson": reverse(
                "admin_api:v1:public-query-api-geojson", kwargs={"pk": "--url_code--"}
            ),
        }
        api_config = {
